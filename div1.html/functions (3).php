...
text/x-generic functions.php ( PHP script, UTF-8 Unicode text, with very long lines )
<?php
            ...
            if ($lead_id) {
                ...
                if ($update['http_code'] === 200) {
                    ...
                    $retry_data = array_filter($leadForUpdate, function($item) use ($exception) {
                        if (stripos($exception, needle: 'Email') !== false && $item['Attribute'] === 'EmailAddress') return false;
                        if (stripos($exception, needle: 'Phone') !== false && $item['Attribute'] === 'Phone') return false;
                        return true;
                    });
